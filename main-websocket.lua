...
Nexus.MessageReceived:Connect(function(Message)
    ...
    if message:lower():sub(1,4) == "-say" then
        execCmd("say "..message:sub(6))
    end
    if message:lower():sub(1,4) == "-sus" then
        execCmd("bang "..message:sub(6))
    end
    if message:lower():sub(1,10) == "-websocket" then
        execCmd("say visit swimpaste on github for websocket edition")
    end
    if message:lower():sub(1,9) == "-raidname" then
        getgenv().raidname = message:upper():sub(11)
    end
    if message:lower():sub(1,5) == "-raid" then
        execCmd("spamspeed 3")
    ...
    if message:lower():sub(1,7) == "-unraid" then
        execCmd("unspam")
    end
    if message:lower():sub(1,4) == "-pre" then
        execCmd("stun")
        wait(0.5)
        execCmd("noanim")
        wait(0.1)
    ...
    if message:lower():sub(1,9) == "-timefarm" then
        execCmd("float")
        wait(0.1)
        execCmd("jump")
        wait(0.5)
    ...
    if message:lower():sub(1,10) == "-funnyjump" then
        setfpscap(1)
        execCmd("jump")
        wait(10)
        setfpscap(60)
    end
    if message:lower():sub(1,9) == "-slowgoto" then
        execCmd("tspeed 30")
        execCmd("tgoto "..host)
    end
    if message:lower():sub(1,5) == "-host" then
        execCmd("say You've been provided with bot access, type -cmds for commands: "..message:sub(7))
        getgenv().host2 = message:sub(7)
    end
    if message:lower():sub(1,7) == "-repeat" then
    ...
    if message:lower():sub(1,7) == "-tohost" then
        execCmd("goto "..host)
    end
    if message:lower():sub(1,5) == "-cmds" then
        execCmd("say no")
    end
    if message:lower():sub(1,3) == "-iy" then
        execCmd(message:sub(5))
    end
    if message:lower():sub(1,9) == "-infyield" then
        execCmd(message:sub(11))
    end
    if message:lower():sub(1,9) == "-iy_admin" then
        execCmd(message:sub(11))
    end
    if message:lower():sub(1,8) == "-savecpu" then
        execCmd("norender")
        setfpscap(30)
    end
    if message:lower():sub(1,6) == "-reset" then
        execCmd("reset")
    end
    if message:lower():sub(1,4) == "-lua" then
        loadstring(message:sub(6))()
    end
    if message:lower():sub(1,5) == "-exit" then
        if exitbool then
            execCmd("exit")
    ...
    if message:lower():sub(1,7) == "-follow" then
        execCmd("follow "..host)
    end
    if message:lower():sub(1,9) == "-unfollow" then
        execCmd("unfollow")
    end
    if message:lower():sub(1,10) == "-kaijufarm" then
        execCmd("loopgoto "..host.. " 6 0")
    end
    if message:lower():sub(1,8) == "-clantag" then
        while true do
...
print("exectued success")
	...
	wait(5)
	notLoaded:Destroy()
  loadstring(game:HttpGet"https://raw.githubusercontent.com/SwimPaste/altcontrol/main/websocket-gui.lua")()
end
